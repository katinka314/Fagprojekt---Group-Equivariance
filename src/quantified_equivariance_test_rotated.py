...
# ---------------- FREE PARAMETERS ----------------
WEIGHTS = {
    "CNN":    ROOT / "reports/param_datasize_2/CNN/datasize_0.03/channels_16/weights_seed3.pth",
...
def plot_feature_grid(model, name, target_layer, channel, img, img_idx, angles=(0, 22.5, 45,  90)):
    ...
    fig.savefig(OUT_DIR / f"ROT_{name}_img{img_idx}_L{target_layer}_ch{channel}_feature_plot.png", dpi=200)
    plt.close(fig)
...
def plot_class_scores(mat, name, idx, kind, cls):     # mat [N_angles,10]; cls = true class of the image
    ...
    plt.savefig(OUT_DIR / f"ROT_{name}_img{idx}_classscores_{kind}.png", dpi=200); plt.close()
